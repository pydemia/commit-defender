...
/** Factory: returns the appropriate runner based on the configured mode. */
export function createRunner(cfg: ExtensionConfig): DockerRunner | LocalRunner {
  return cfg.runnerMode === 'local' ? new LocalRunner(cfg) : new DockerRunner(cfg);
}
...
/**
 * Runs `python -m commit_defender.entrypoint` (or `commit-defender` CLI) directly
 * on the host, without Docker. Requires the commit_defender package to be installed
 * in the Python environment pointed to by `cfg.pythonPath`.
 */
export class LocalRunner {
  ...
  run(repoRoot: string, stagedFiles: string[]): Promise<RunResult> {
    return new Promise((resolve, reject) => {
      ...
      channel.appendLine(`\n[Commit Defender] Running: ${python} ${args.join(' ')}`);
      ...
      try {
        ...
        const msg = (err as NodeJS.ErrnoException).code === 'ENOENT'
          ? `Python not found at '${python}'. Set commitDefender.pythonPath in settings.`
          : String(err);
        return reject(new Error(msg));
      }
      ...
      proc.on('close', () => {
        ...
        if (!stdout.trim()) {
          reject(new Error(
            'No JSON output from commit-defender.\n' +
            'Make sure the package is installed:\n' +
            `  ${python} -m pip install -e <path-to-commit-defender>`
          ));
          return;
        }
