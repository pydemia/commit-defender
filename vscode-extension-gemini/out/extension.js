...
function activate(context) {
    ...
    (0, installer_js_1.ensurePackageInstalled)(cfg.pythonExecutable, extensionVersion, context).catch(() => { });
    ...
    const hoverProvider = new hoverProvider_js_1.SuggestionHoverProvider();
    ...
    context.subscriptions.push(diagnostics, commentCtrl, statusBar.item, historyView, vscode.languages.registerCodeLensProvider(ALL_FILES, codeLensProvider), vscode.languages.registerHoverProvider(ALL_FILES, hoverProvider));
    ...
    async function analyze(relPaths, repoRoot) {
        ...
        (0, diagnostics_js_1.applyDiagnostics)(result.report, repoRoot, diagnostics);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('commitDefender.analyzeCurrentFile', async () => {
    ...
    // ── 2. Analyze Directory ───────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand('commitDefender.analyzeDirectory', async () => {
        const ws = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!ws) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('commitDefender.showLineSuggestion', (uri, line0) => {
        const set = findingsStore_js_1.findingsStore.get(uri);
        if (!set) {
            return;
        }
        ...
        if (lintFindings.length) {
            ...
            html += lintFindings.map(f => `<li><span class="sev-${esc(f.severity)}">${esc(f.severity)}</span> ` +
                `<code>${esc(f.rule)}</code> ${esc(f.message)}</li>`).join('');
            html += '</ul>';
        }
...
function buildSummaryHtml(report, repoRoot) {
