...
import { Locale, RichnessLevel, SeverityLevel } from '../config.js';
...
export const SEVERITY_MIN_RANK: Record<SeverityLevel, number> = {
  lean: 3,
  generous: 2,
  moderate: 1,
  rigorous: 1,
...
const RICHNESS_PROMPTS: Record<RichnessLevel, string> = {
  colorful:
    'For each finding, provide an elaborate explanation: describe the problem in depth, ' +
  ...
  moderate:
    'Provide clear, concise explanations for each finding. Keep the summary under 300 words.',
  simple:
    'Be brief. One or two sentences per finding. Keep the summary under 150 words.',
  silent:
...
export const COMMIT_MESSAGE_SYSTEM_PROMPT = `# Git Commit Message Generation Prompt
