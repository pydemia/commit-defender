...
const artifact = process.env.CD_OVERLAP_EXTENSION_PATH;
const evidence = process.env.CD_OVERLAP_EVIDENCE;
const executable = process.env.CD_OVERLAP_CODEX ?? "/opt/homebrew/bin/codex";
for (const value of [artifact, evidence, executable])
  assert(value && path.isAbsolute(value));
assert.equal(
  process.env.CD_OVERLAP_ALLOW_MODEL,
  "1",
  "Explicit current-account model verification required",
);
const hash = (value) => createHash("sha256").update(value).digest("hex");
const temporary = await realpath(
  await mkdtemp(path.join("/tmp", "cd-overlap-")),
);
const root = path.join(temporary, "workspace");
const profileId = `cd-overlap-${randomUUID()}`;
...
git("add", ".");
git("commit", "-m", "Owned trigger overlap baseline");
...
try {
  await runTests({
    ...
    extensionTestsPath: path.resolve("out-test/trigger-overlap-host.cjs"),
    extensionTestsEnv: {
      CD_OVERLAP_WORKSPACE: root,
      CD_OVERLAP_PROFILE: profileId,
      CD_OVERLAP_EVIDENCE: evidence,
      CD_OVERLAP_INVOCATIONS: invocations,
    },
    launchArgs: [
      root,
      "--user-data-dir",
  ...
  if (proof.status === "verified" && nativeExitObserved && !live.length) {
    ...
    assert.equal(
      proof.hookDirectory,
      path.join(defaultLocalDataDirectory(), "managed-hooks", hash(configPath)),
    );
