...
class PanelProvider {
    ...
    _decoEmitter = new vscode.EventEmitter();
    decorationProvider = {
    ...
    getTreeItem(node) {
        switch (node.kind) {
            ...
            case 'block': {
                ...
                const author = (0, commentFormatter_js_1.formatCategory)(b.category);
                ...
                item.resourceUri = node.uri;
                item.iconPath = new vscode.ThemeIcon(PRIORITY_ICON[b.priority], new vscode.ThemeColor(PRIORITY_COLOR_ID[b.priority]));
                const lineRef = b.line > 0
                    ? `Ln ${b.line}${b.col ? `, Col ${b.col}` : ''}`
                    : 'file-level';
                item.description = lineRef;
                item.tooltip = new vscode.MarkdownString(`**${meta.emoji} ${b.priority} ${meta.label}** · _@${author}_\n\n${b.comment}`);
                item.command = {
                    command: 'vscode.open',
                    title: 'Open',
                    arguments: [
                        ...
                        {
                            selection: new vscode.Range(Math.max(0, b.line - 1), Math.max(0, (b.col ?? 1) - 1), Math.max(0, b.line - 1), Math.max(0, (b.col ?? 1) - 1)),
                            preserveFocus: false,
                            preview: true,
                        },
                    ],
            ...
            default: {
                ...
                item.iconPath = new vscode.ThemeIcon(this._isRunning ? 'loading~spin' : 'shield', new vscode.ThemeColor('charts.blue'));
                item.id = node.id;
                return item;
            }
        }
    ...
    getChildren(node) {
        ...
        if (node.kind === 'file') {
            return node.blocks
                .slice()
                .sort((a, b) => {
                const pr = commentFormatter_js_1.PRIORITY_RANK[b.priority] - commentFormatter_js_1.PRIORITY_RANK[a.priority];
                if (pr !== 0) {
                    return pr;
                }
    ...
    _buildRoot() {
        ...
        const files = Array.from(byFile.entries())
            ...
            const ra = wa ? commentFormatter_js_1.PRIORITY_RANK[wa] : -1;
            const rb = wb ? commentFormatter_js_1.PRIORITY_RANK[wb] : -1;
            if (ra !== rb) {
                return rb - ra;
            }
    ...
    _rebuildDecorations() {
        ...
        sortedFiles.forEach(([, blocks], idx) => {
            ...
            this._decorations.set(fileUri.toString(), {
                ...
                tooltip: `Worst: ${worst} ${types_js_1.PRIORITY_META[worst].label}`,
            });
        });
        ...
        sortedFiles.forEach(([, blocks], fileIdx) => {
            ...
            sorted.forEach((b, idx) => {
                ...
                this._decorations.set(uri.toString(), {
                    ...
                    tooltip: `${meta.label}${b.category ? ` · ${(0, commentFormatter_js_1.formatCategory)(b.category)}` : ''}`,
                });
            });
        });
...
function worstPriority(blocks) {
    ...
    for (const b of blocks) {
        if (!worst || commentFormatter_js_1.PRIORITY_RANK[b.priority] > commentFormatter_js_1.PRIORITY_RANK[worst]) {
            worst = b.priority;
        }
    }
    return worst;
