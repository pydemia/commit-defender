...
Whole-file unstaging and no-op index events are ignored; partial-hunk unstaging can still schedule a review. Open a nested Git repository as its own workspace folder to give it a separate event observer. Automatic service configuration currently requires an online central selection when using centralized mode. The service uses the explicitly registered CD client binding for that connection; ordinary CLI commands still require a CLI binding. User-wide token budgets, manual/service scheduling priority, retention and complete editor-startup/crash recovery remain pending. Interrupted jobs with saved reports can be inspected with **Commit Defender: Recover Background Review**.
